...
object BlockJar extends BlockJar("jar") with RegistryHandler[Block] {
...
class BlockJar(name: String)
  ...
  override def initializeItemBlock() = {
    this.itemBlock =
      new ItemBlockJar(this).setRegistryName(this.getRegistryName())
    BlockBase.itemBlocks += this.itemBlock
    this
  }

  override def getStateFromMeta(meta: Int) = this
    .getDefaultState()
    .withProperty(FACING, if (meta == 0) EnumFacing.NORTH else EnumFacing.SOUTH)

  override def getMetaFromState(state: IBlockState) =
    if (state.getValue(FACING) == EnumFacing.NORTH) 0 else 1
...
//	}
  ...
  override def getWeight(itemStack: ItemStack) = Option(
