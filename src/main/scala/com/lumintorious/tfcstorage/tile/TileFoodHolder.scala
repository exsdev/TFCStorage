...
class TileFoodHolder extends TileEntity with ITickable {
  ...
  def fits(stack: ItemStack): Boolean =
    stack.hasCapability(CapabilityFood.CAPABILITY, null) &&
      (this.stack.isEmpty || (this.stack.getItem() == stack
        .getItem() && (!this.stack.getHasSubtypes() || this.stack
        .getMetadata() == stack.getMetadata()) && ItemStack
        .areItemStackTagsEqual(this.stack, stack)))
