...
class TileJar extends TileFoodHolder with ICapabilityProvider {
  ...
  def loadFromStack(stack: ItemStack): Unit = {
    val tag = Option(stack.getTagCompound).getOrElse(
      return
    )
    this.fluidTank.setFluid(
  ...
  def canPreserve(stack: ItemStack): Boolean = {
    ...
    if (
      fluidStack.getFluid() != FluidsTFC.VINEGAR
        .get() || fluidStack.amount < stack.getCount() * 125
    ) {
      false
    } else {
  ...
  override def readFromNBT(nbt: NBTTagCompound) = {
    this.fluidTank.setFluid(
      FluidStack.loadFluidStackFromNBT(nbt.getCompoundTag("fluid"))
    )
