      ...
      case food: IFood =>
        if (food.isRotten()) {
          ...
          if (rottenDate == Long.MaxValue) {
            ...
            // Date food rots on.
            val rottenCalendarTime = rottenDate - CalendarTFC.PLAYER_TIME
              .getTicks() + CalendarTFC.CALENDAR_TIME.getTicks();
            // Days till food rots.
            val daysToRotInTicks =
              rottenCalendarTime - CalendarTFC.CALENDAR_TIME.getTicks();
            list += (
              I18n.format(
                "tfc.tooltip.food_expiry_date.days",
