...
import com.lumintorious.tfcstorage.block.{
  BlockBase,
  BlockGrainPile,
  BlockHanger,
  BlockJar,
...
object EventHandler {
  ...
  def onItemUse(event: LivingEntityUseItemEvent.Stop): Unit = {
    val stack =
      event.getEntityLiving().getItemStackFromSlot(EntityEquipmentSlot.MAINHAND)
    val name =
      stack.getItem().getRegistryName().getPath().split("/").last.toLowerCase()
    if (event.getEntityLiving().isSneaking() && name.contains("_grain")) {
      ...
      if (rayTrace.typeOfHit == RayTraceResult.Type.BLOCK) {
        ...
        if (world.getBlockState(pos) == Blocks.AIR.getDefaultState()) {
          val block = BlockGrainPile.all.getOrElse(
            name,
            return
          )
